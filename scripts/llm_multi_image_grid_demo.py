r"""scripts/llm_multi_image_grid_demo.py

Quick probe: can the selected vision model handle multiple images in one prompt?

This script runs TWO independent branches:
- Branch A: 8x8 grid (64 images) for 2 rounds
- Branch B: 3x3 grid (9 images) for 4 rounds

Each branch starts from the same initial fullscreen screenshot (captured once),
then repeatedly slices the currently-selected tile and asks:
"In what picture is the Chrome browser icon?"
...
def _question_for_grid(n: int) -> str:
    max_tiles = n * n
    return (
        "In what picture is the Chrome browser icon? "
...
def _critic_question(user_question: str) -> str:
    return (
        "CRITIC CHECK: Given ONLY this single image tile, answer yes/no: "
        f"Is the thing the user looked for really present in this tile?\n\nUser question: {user_question}"
    )
...
def _parse_yes_no(text: str) -> bool | None:
    ...
    if s.startswith("no"):
        return False
    # tolerate single-letter answers
    if s == "y":
        return True
    if s == "n":
        return False
...
def _save_full_image_with_circle(
    *,
    ...
    # Optional prompt banner at the top-center.
    banner = (prompt_text or "").strip()
    if banner:
        # Try to get a noticeably larger font (about ~5x default).
        font = None
        try:
        ...
        # Pillow compatibility: `textbbox` is preferred.
...
def main() -> int:
    ...
    def run_branch(*, branch: str, grid_n: int | list[int], rounds: int) -> bool:
        ...
        try:
            ...
            # Cumulative offsets are computed in *downscaled* coordinates.
            offset_x = 0
            offset_y = 0
            ...
            for r in range(1, rounds + 1):
                ...
                question = _question_for_grid(this_grid_n)
                ...
                while True:
                    ...
                    # We need dimensions to compute the chosen tile bounds.
                    from PIL import Image  # type: ignore
                    ...
                    try:
                        ...
                        # If round 1 succeeded but this round failed, fall back to round 1 center.
                    ...
                    if choice is None:
                        # Adaptive: bump temperature and retry within budget.
                    ...
                    # Update cumulative offsets based on which tile was selected this round.
                    ...
                    # Critic step: confirm the target is actually present in the chosen tile.
                    try:
                        critic_inp = [
                            {
                                "role": "user",
                                "content": _build_single_image_question(
                                    ...
                                    question=_critic_question("In what picture is the Chrome browser icon?"),
                                ),
                            }
                        ]
                ...
                # If we broke out of the retry loop due to fallback, stop processing more rounds.
            ...
            # Primary center from the last completed round; otherwise fallback to round 1.
            ...
            # Map back to full-resolution coordinates for drawing.
            ...
            try:
                _save_full_image_with_circle(
                    ...
                    center_x=center_x,
                    center_y=center_y,
                    prompt_text="In what picture is the Chrome browser icon?",
                )
                print(
                    f"\nSaved full image w/ center marker: {full_out_path} "
                    f"(center_small=({center_x_small},{center_y_small}) -> center_full=({center_x},{center_y}))"
                )
            except Exception as e:
                print(f"\nWARNING: Failed to save full image overlay for branch {branch}: {e}", file=sys.stderr)
