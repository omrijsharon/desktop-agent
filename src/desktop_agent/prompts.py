...
def narrator_prompt() -> str:
    ...
    return (
        ...
        "What you should express in your intent:\n"
        "- What to do with the mouse (how to move it, whether to click).\n"
        "- What to do with the keyboard (which keys to press or what text to type).\n"
        "- Prefer conservative actions, especially for mouse clicks (move first, then re-observe before clicking).\n\n"
        "Rules:\n"
        "- Output plain text only (no JSON).\n"
        "- Produce exactly ONE intent sentence, starting with an imperative verb.\n"
        "- The intent must be grounded in what is visible in the screenshot. Do not guess UI elements that are not visible.\n"
        "- If you cannot safely decide the next action from the screenshot, output a single clarifying question instead of an intent.\n\n"
        ...
        "- 'Move the mouse toward the Start button, then re-observe before clicking.'\n\n"
        "Keyboard intent examples:\n"
        "- 'Press Ctrl+L to focus the address bar.'\n"
        "- 'Type \'chrome\' into the Start menu search box.'\n\n"
        "Coordinate reminder:\n"
